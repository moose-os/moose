...
//!
//! ## Notes
//! - Scancode translation to characters is performed in the guest (see `SCANCODE_MAP`).
//! - The protocol supports multiple consecutive key events without requiring delays.
//! - The protocol supports turning on and off LED lights on the keyboard, but this is not yet implemented.
//!
use phf::phf_map;

use crate::driver::hv::hyperv::{
    channel::VmBusChannel, synthetic_device::VmBusSyntheticDevice, VmBusOfferChannel,
    VmBusPacketType,
};
...
pub struct VmBusKeyboard {
    ...
    pub(crate) offer: VmBusOfferChannel,
}
...
impl VmBusSyntheticDevice for VmBusKeyboard {
    fn initialize(&self) -> bool {
        ...
        let mut init = KeyboardProtocolRequest {
        ...
        self.channel.send_packet(
            unsafe { &init as *const _ as *const u8 },
        ...
        loop {
            ...
            match message_type {
                ...
                KeyboardMessageType::Event => {
                    ...
                    let is_unicode = (kbd_pkt.flags & 1) != 0;
                    let is_break = (kbd_pkt.flags & 2) != 0;
                    let is_e0 = (kbd_pkt.flags & 4) != 0;
                    let is_e1 = (kbd_pkt.flags & 8) != 0;
                    let code = kbd_pkt.code;

                    log::debug!("Keyboard event: char={chr} scancode=0x{code:x} - unicode={is_unicode},break={is_break},e0={is_e0},e1={is_e1}");
                }
                _ => unreachable!(),
            }
        }
