...
//! ## Packet Layout
//!
//! All mouse packets are encapsulated within a **pipe header**, which contains:
//! - `message_type`: type of message
//! - `size`: size of the actual mouse packet data  
//!
//! The pipe header ensures correct framing and multiplexing over the VMbus channel.
//!
//! ```text
//! +----------------------+------------------------+--------------------------+
//! | VmBusMousePipeHeader | VmBusMousePacketHeader |    Packet-specific Data  |
//! +----------------------+------------------------+--------------------------+
//! | message_type: 4 bytes| packet_type: 4 bytes   | e.g. buttons: 1 byte     |
//! | size: 4 bytes        | length: 4 bytes        | x: 2 bytes               |
//! | (total 8 bytes)      | (total 8 bytes)        | y: 2 bytes               |
//! |                      |                        | wheel: 1 byte            |
//! |                      |                        | pan: 1 byte              |
//! +---------------------+----------------------+-----------------------------+
//! ```
//! Total size varies depending on packet-specific data length.
//!
//! - VmBusMousePipeHeader frames the whole mouse packet.
//! - VmBusMousePacketHeader describes the mouse packet type and length.
//! - Packet-specific data follows, e.g. in InputReport: buttons, x/y movement, wheel, pan.
//!
//! ## InputReport Packet
//!
...
use crate::driver::hv::hyperv::{
    ...
    synthetic_device::{mouse, VmBusSyntheticDevice},
    VmBusOfferChannel,
};
...
/// Defines the types of pipe messages
#[derive(Debug, Copy, Clone, Eq, PartialEq)]
#[repr(u32)]
enum VmBusMousePipeMessageType {
    /// Invalid or uninitialized message.
    Invalid = 0,

    /// Standard data message containing mouse input or related information.
    Data = 1,
}

/// Header structure for VMBus synthetic mouse pipe messages.
///
/// This header precedes the actual mouse data payload and describes the type  
/// of the message and its size.
#[derive(Debug, Copy, Clone)]
#[repr(C, packed)]
struct VmBusMousePipeHeader {
    /// Type of the pipe message
    message_type: VmBusMousePipeMessageType,
...
impl VmBusSyntheticDevice for VmBusMouseDriver {
    ...
    fn process_incoming_data(&self) {
        ...
        loop {
            ...
            let (header, data_slice) = packet.unwrap();
            let data = data_slice.as_ptr();

            let pipe_hdr = unsafe { data as *const _ as *const VmBusMousePipeHeader };
            let mouse_packet =
...
impl VmBusMouseDriver {
    fn send_packet<T>(&self, packet: &T) {
        let pipe = VmBusMousePipeHeader {
            message_type: VmBusMousePipeMessageType::Data,
            size: size_of::<T>() as u32,
        };

        let buffer_len = size_of::<VmBusMousePipeHeader>() + size_of::<T>();

        let mut buffer = vec![0u8; buffer_len];
        let mut buffer_ptr = buffer.as_mut_ptr();
        unsafe {
            ptr::copy(
                &pipe as *const _ as *const u8,
                buffer_ptr,
                size_of::<VmBusMousePipeHeader>(),
            );
            ptr::copy(
                packet as *const _ as *const u8,
                buffer_ptr.add(size_of::<VmBusMousePipeHeader>()),
                size_of::<T>(),
            );
        }
        ...
        self.channel.send_packet(
            ...
            1,
            true,
            crate::driver::hv::hyperv::VmBusPacketType::DataInband,
        );
    }
}
