...
use crate::{
    driver::hv::hyperv::{
        ...
        VmBusOfferChannel, VmBusPacketHeader, VmBusPacketType, HYPERV_PAGE_SIZE,
    },
    memory::{memory_manager, Frame, Page, PageFlags, PhysicalAddress, VirtualAddress},
};
...
/// Base virtual address of the synthetic video framebuffer in guest memory.
const SYNTHETIC_VIDEO_FRAMEBUFFER_BASE: u64 = 0x5A00_0000_0000;
...
/// Synthetic Video *Pipe* message types.
#[derive(Debug, Copy, Clone)]
#[repr(u32)]
enum SyntheticVideoPipeMessageType {
    Data = 1,
}

/// Header for messages sent through the Synthetic Video channel.
#[derive(Debug, Copy, Clone)]
#[repr(C, packed)]
struct SyntheticVideoPipeHeader {
    ///  Specifies the type of message being sent through the pipe.  
    message_type: SyntheticVideoPipeMessageType,
...
impl VmBusSyntheticDevice for VmBusSyntheticVideoDriver {
    fn initialize(&self) -> bool {
        ...
        // transation system with waiting for answers. For every error in protocol negotiation, we panic at
        ...
        // Map framebuffer at SYNTHETIC_VIDEO_FRAMEBUFFER_BASE
                ...
                memory_manager()
                    ...
                    .map_for_current_address_space(
                        &Page::new(VirtualAddress::new(
                            SYNTHETIC_VIDEO_FRAMEBUFFER_BASE + i as u64 * HYPERV_PAGE_SIZE as u64,
                        )),
                        &Frame::new(PhysicalAddress::new(
                            framebuffer_frame.address().as_u64()
                                + i as u64 * HYPERV_PAGE_SIZE as u64,
                        )),
    ...
    fn process_incoming_data(&self) {
        ...
        loop {
            ...
            let (packet_header, data) = packet.unwrap();

            let pipe_header =
                unsafe { *(data.as_ptr() as *const _ as *const SyntheticVideoPipeHeader) };
            let synthvid_data_ptr = unsafe { data.as_ptr().add(size_of::<SyntheticVideoHeader>()) };
            ...
            match synthvid_header.message_type {
                ...
                SyntheticVideoMessageType::FeatureChange => {
                    let pkt = unsafe { *(synthvid_data_ptr as *const SyntheticVideoFeatureChange) };
...
impl VmBusSyntheticVideoDriver {
    ...
    fn send_packet<T>(&self, packet: &T) {
        let pipe = SyntheticVideoPipeHeader {
            message_type: SyntheticVideoPipeMessageType::Data,
            size: size_of::<T>() as u32,
        };

        let buffer_len = size_of::<SyntheticVideoPipeHeader>() + size_of::<T>();
        ...
        unsafe {
            ptr::copy(
                ...
                buffer_ptr,
                size_of::<SyntheticVideoPipeHeader>(),
            );
            ptr::copy(
                packet as *const _ as *const u8,
                buffer_ptr.add(size_of::<SyntheticVideoPipeHeader>()),
                size_of::<T>(),
            );
        }

        let xid = without_interrupts(|| self.state.read().next_xid.fetch_add(1, Ordering::Relaxed));
