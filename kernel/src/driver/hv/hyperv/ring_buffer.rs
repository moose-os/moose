...
impl HyperVSingleRingBuffer {
    ...
    pub fn new(starting_frame: &Frame, size_in_bytes: u32) -> HyperVSingleRingBuffer {
        ...
        let buffer_pages = size_in_bytes / HYPERV_PAGE_SIZE as u32;

        // Map header somewhere in memory
        let header = unsafe {
            memory_manager().write().map_any_for_current_address_space(
                starting_frame,
                PageFlags::WRITABLE | PageFlags::DISABLE_CACHING,
            )
        };

        // Map actual data pages in memory just after the header
        for i in 1..buffer_pages as u64 {
            unsafe {
                memory_manager().write().map_for_current_address_space(
                    &Page::new(VirtualAddress::new(
                        header.address().as_u64() + i * HYPERV_PAGE_SIZE as u64,
                    )),
                    &Frame::new(PhysicalAddress::new(
                        starting_frame.address().as_u64() + i * HYPERV_PAGE_SIZE as u64,
                    )),
                    PageFlags::WRITABLE | PageFlags::DISABLE_CACHING,
                );
            };
        ...
        // Map first page just after last to avoid overlapping logic
        // @TODO: Need to map only the first page or whole buffer?
        unsafe {
            memory_manager().write().map_for_current_address_space(
                &Page::new(VirtualAddress::new(
                    header.address().as_u64() + (buffer_pages as u64) * HYPERV_PAGE_SIZE as u64,
                )),
                &Frame::new(PhysicalAddress::new(
                    starting_frame.address().as_u64() + HYPERV_PAGE_SIZE as u64,
                )),
                PageFlags::WRITABLE | PageFlags::DISABLE_CACHING,
            );
        };
        ...
        HyperVSingleRingBuffer {
            header: header.address().as_mut_ptr(),
            data_start: unsafe { header.address().as_mut_ptr::<u8>().add(HYPERV_PAGE_SIZE) },
            size_in_bytes,
        }
    }
...
impl HyperVDoubledRingBuffer {
    ...
    pub fn send(&self, data: *const u8, len: usize, mut write_offset: u32) -> u32 {
        unsafe { copy(data, self.tx.data_start.add(write_offset as usize), len) };

        write_offset += len as u32;
        write_offset %= self.tx.ring_buffer_data_size() as u32;
