...
const HPET_CAPABILITIES_REGISTER: isize = 0x00;

/// General Configuration Register
const HPET_CONFIGURATION_REGISTER: isize = 0x10;

/// Main Counter Value Register
const HPET_MAIN_COUNTER_REGISTER: isize = 0xF0;

/// Target 10ms expressed in femtoseconds (10^13).
...
impl ClockSource for HpetTimer {
    ...
    fn measure_tsc_ticks(&self) -> u64 {
        ...
        // Read capabilities register to get the counter period (higher 32 bits) in femtoseconds (10^-12 s)
        let cap_reg =
            unsafe { read_volatile(hpet_base.offset(HPET_CAPABILITIES_REGISTER) as *const u64) };
        let period_fs = cap_reg >> 32;

        if period_fs == 0 {
            error!("HPET counter period is 0. Broken hardware/emulation.");
        ...
        let mut config =
            unsafe { read_volatile(hpet_base.offset(HPET_CONFIGURATION_REGISTER) as *const u64) };
        config |= 1; // ENABLE_CNF
        unsafe {
            write_volatile(
                hpet_base.offset(HPET_CONFIGURATION_REGISTER) as *mut u64,
                config,
            )
        };
        ...
        let start_hpet =
            unsafe { read_volatile(hpet_base.offset(HPET_MAIN_COUNTER_REGISTER) as *const u64) };
        ...
        loop {
            let current_hpet = unsafe {
                read_volatile(hpet_base.offset(HPET_MAIN_COUNTER_REGISTER) as *const u64)
            };
